        ...
        <div class="panel-body">
            ...
            @csrf
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-9">
                        <select name="role" id="role" class="select-search">
                            @foreach($role as $roles)
                            <option value="{{$roles->id}}" {{ (collect(old('role'))->contains($roles->id)) ? 'selected':'' }}@if($data->roles->pluck('id')->first() == $roles->id) selected='selected' @endif >{{$roles->name}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('role'))
                        <label style="padding-top:7px;color:#F44336;">
